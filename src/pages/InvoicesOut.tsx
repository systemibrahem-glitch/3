...
import type { InvoiceOut, Partner } from '@/lib/supabase';
...
export default function InvoicesOut() {
  ...
  const [editingInvoice, setEditingInvoice] = useState<InvoiceOut | null>(null);
  ...
  function openEditDialog(invoice: InvoiceOut) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">الصادرات</h1>
          <p className="text-slate-600 dark:text-slate-400">إدارة فواتير الصادرات والمبيعات</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingInvoice ? 'تعديل فاتورة' : 'إضافة فاتورة صادر'}</DialogTitle>
            </DialogHeader>
      ...
      <Card className="p-6">
        ...
        <div className="mb-4 flex gap-4 flex-wrap">
          {Object.entries(totalByCurrency).map(([currency, total]) => (
            <div key={currency} className="px-4 py-2 bg-red-50 dark:bg-red-900/20 rounded-lg">
              <span className="text-sm text-slate-600 dark:text-slate-400">الإجمالي: </span>
              <span className="font-bold text-red-600">{formatCurrency(total, currency)}</span>
            </div>
          ))}
        </div>
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-left">
                    <p className="text-lg font-bold text-red-600">{formatCurrency(invoice.amount, invoice.currency)}</p>
                  </div>
                </div>
              </div>
